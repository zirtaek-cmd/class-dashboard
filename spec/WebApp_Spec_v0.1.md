...
| 공지사항 | Firestore (`notices`) | 교사 작성, 수정 전까지 계속 표시, 반/학년/전교 3단계 범위 |
...
```
# 반별 공지 (11개)
notices/{classId}  { text: string }

# 학년 공지 (3개) — 신규
notices/grade-1, notices/grade-2, notices/grade-3  { text: string }

# 전교 공지 (1개) — 신규
notices/all  { text: string }
...
- **공지 지속성**: 별도의 자동 초기화 로직 없음 — 교사가 새 내용으로 저장하기 전까지 화면에 계속 표시됨(알림의 자정 정리 로직과는 무관)
...
- 대상 선택 시 **해당 대상에 저장된 공지를 입력창에 자동으로 불러옴**
...
- 각 문서 중 내용이 비어있지 않은 것만 선별하여 **하나의 공지 보드에 통합 표시** (날짜와 무관하게 수정 전까지 계속 표시)
...
- 알림 메모에 포함되는 실명 등 개인정보는 **당일 한정, 최소 보관** — 확인 완료 시 메모 내용 즉시 삭제(이력 미보관)
- 공지사항은 교사가 새로 저장하기 전까지 **날짜 제한 없이 계속 게시**되므로, 실명 등 개인정보를 포함하지 않도록 작성 시 유의(작성 교사 책임)
